...
//
// end.Call("OmniPaxos.AppendEntries", &args, &reply) -- send an RPC, wait for reply.
// the "OmniPaxos" is the name of the server struct to be called.
...
//
// srv := MakeServer()
// srv.AddService(svc) -- a server can have multiple services, e.g. Ominpaxos and k/v
...
type reqMsg struct {
	...
	svcMeth  string      // e.g. "OmniPaxos.AppendEntries"
...
func (rn *Network) processReq(req reqMsg) {
	...
	if enabled && servername != nil && server != nil {
		...
		ms := 0
		if rn.longDelays {
			// let omnipaxos tests check that leader doesn't send
			// RPCs synchronously.
			ms = (rand.Int() % 7000)
		} else {
...
// so that e.g. both omnipaxos and ak/v server can listen to the same rpc endpoint.
type Server struct {
	mu       sync.Mutex
...
func (rs *Server) dispatch(req reqMsg) replyMsg {
	...
	// split OmniPaxos.AppendEntries into Service and method
